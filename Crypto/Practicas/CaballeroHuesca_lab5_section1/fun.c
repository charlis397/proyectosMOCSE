...
/*void generarLlave(int longitud){
	...
	int i,j,tamanio;
	unsigned char llave[longitud][N];
	unsigned char aux[longitud][N];
...
void expandirClave(char* archivoSalida,unsigned char (*arrClave)[4], unsigned char *mClaveExp){
	...
	for(i=4;i < 44;i++){
		...
		if (i % 4 == 0){  //cada cuatro filas
		  ...
		  copia[0] =  (copia[0] ^ rCon[i/4])-1;  //suma bit a bit cte rcon  
		  printf("\n\n\n");
		  fprintf(fp,"\n\n\n");
